...
(function () {
  ...
  // Interner Werkzeugkasten für die ausgelagerten Teilmodule (js/app-*.js).
  // Befüllt wird er am Ende dieser Datei; dort steht auch, wer was braucht.
  // Kein Build-Schritt, keine ES-Module — die Teilmodule sind klassische
  // <script src>-Dateien, die nach app.js geladen werden. Weil app.js erst bei
  // DOMContentLoaded startet (also nach ALLEN Skripten), ist die Reihenfolge
  // zwischen den Teilmodulen untereinander unkritisch.
  var HIntern = window.HonIntern = window.HonIntern || {};
  ...
  // ───── Manga- und Zeitschriften-Quellen (AniList, Jikan, DNB, Verlage) → js/app-quellen.js ─────
  // Weiterleitungen, damit jede Aufrufstelle unten unveraendert bleibt.
  function alRelated() { return HIntern.alRelated.apply(null, arguments); }
  function alSearch() { return HIntern.alSearch.apply(null, arguments); }
  function jikanSearch() { return HIntern.jikanSearch.apply(null, arguments); }
  function searchBooks() { return HIntern.searchBooks.apply(null, arguments); }
  function searchMagazines() { return HIntern.searchMagazines.apply(null, arguments); }
  function searchMangas() { return HIntern.searchMangas.apply(null, arguments); }
  ...
  // ───── Statistik, Besitz, Lese-Tempo, Tsundoku → js/app-statistik.js ─────
  // Weiterleitungen, damit jede Aufrufstelle unten unveraendert bleibt.
  function allLocations() { return HIntern.allLocations.apply(null, arguments); }
  function dnfHtml() { return HIntern.dnfHtml.apply(null, arguments); }
  function exportQuoteImage() { return HIntern.exportQuoteImage.apply(null, arguments); }
  function forecastHtml() { return HIntern.forecastHtml.apply(null, arguments); }
  function formatRowHtml() { return HIntern.formatRowHtml.apply(null, arguments); }
  function journalHtml() { return HIntern.journalHtml.apply(null, arguments); }
  function money() { return HIntern.money.apply(null, arguments); }
  function ownershipHtml() { return HIntern.ownershipHtml.apply(null, arguments); }
  function parsePrice() { return HIntern.parsePrice.apply(null, arguments); }
  function quotesHtml() { return HIntern.quotesHtml.apply(null, arguments); }
  function renderStats() { return HIntern.renderStats.apply(null, arguments); }
  function shopLinksHtml() { return HIntern.shopLinksHtml.apply(null, arguments); }
  function tsundoku() { return HIntern.tsundoku.apply(null, arguments); }
  ...
  // ───── v1.4: Erfolge / Abzeichen → js/app-erfolge.js ─────
  // Weiterleitungen, damit jede Aufrufstelle unten unveraendert bleibt.
  // ACH_DEFS liegt jetzt in js/app-erfolge.js (ueber HIntern erreichbar)
  function achStats() { return HIntern.achStats.apply(null, arguments); }
  function checkAchievements() { return HIntern.checkAchievements.apply(null, arguments); }
  function loadAch() { return HIntern.loadAch.apply(null, arguments); }

  // ───── v1.4: Jahresrueckblick + Jahres-Duell → js/app-jahr.js ─────
  // Weiterleitungen, damit jede Aufrufstelle unten unveraendert bleibt.
  function openYearDuel() { return HIntern.openYearDuel.apply(null, arguments); }
  function openYearReview() { return HIntern.openYearReview.apply(null, arguments); }
  ...
  // ───── Werkzeugkasten für die Teilmodule füllen ─────
  // Alles hier ist entweder eine Funktionsdeklaration (stabile Referenz) oder
  // eine Konstante. VERÄNDERLICHE Variablen dürfen NICHT hierher — ein Teilmodul
  // bekäme nur eine Kopie und sähe spätere Änderungen nicht.
  HIntern.$ = $;
  HIntern.esc = esc;
  HIntern.toast = toast;
  HIntern.fmtDate = fmtDate;
  HIntern.lib = lib;
  HIntern.bookIndex = bookIndex;
  HIntern.seriesOf = seriesOf;
  HIntern.loadSessions = loadSessions;
  HIntern.loadSettings = loadSettings;
  HIntern.readDatesOf = readDatesOf;
  HIntern.LS_ACH = LS_ACH;
  HIntern.FORMAT_LBL = FORMAT_LBL;
  HIntern.GB = GB;
  HIntern.bookKey = bookKey;
  HIntern.dnbSearch = dnbSearch;
  HIntern.doSearch = doSearch;
  HIntern.gbFetch = gbFetch;
  HIntern.gbSearch = gbSearch;
